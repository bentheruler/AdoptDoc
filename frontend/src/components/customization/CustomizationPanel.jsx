...
import { sendCVMessage } from '../../services/aiService';
...
const CustomizationPanel = ({ cvData, onCVUpdate }) => {
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      const { displayText, updatedCV } = await sendCVMessage(userMsg, cvData);
  ...
  const bubbleStyle = (role) => ({
    ...
    padding: '8px 11px', fontSize: 11.5, lineHeight: 1.5,
    whiteSpace: 'pre-wrap', wordBreak: 'break-word',
  });

  return (
    <div style={{ width: 250, background: '#fff', borderRight: '1px solid #e2e8f0', padding: 16, display: 'flex', flexDirection: 'column', flexShrink: 0, minHeight: 0 }}>
      {/* Header */}
      ...
      <div style={{ marginTop: 10, display: 'flex', gap: 6, alignItems: 'flex-end', flexShrink: 0 }}>
        ...
        <button
          ...
          style={{ background: loading || !input.trim() ? '#cbd5e1' : '#1e3a5f', color: '#fff', border: 'none', borderRadius: 8, width: 36, height: 36, cursor: loading ? 'not-allowed' : 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
        >➤</button>
      </div>
